...
extern "C" void orblib_cpp_api_orbitstart_box_start_records(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        black_hole_softening_arcsec < 0.0 || dark_halo_parameter_count < 0 ||
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        energy_count <= 0 || i2_count <= 0 || i3_count <= 0 || energies == nullptr ||
    ...
    try {
        ...
        if (!dynamite::orblib_cpp::build_box_start_records(
                mge,
                halo,
                black_hole_mass,
                black_hole_softening_arcsec * mge.conversion_factor,
...
extern "C" void orblib_cpp_api_orbitstart_tube_start_records(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    double* records,
    int* noreg_flags,
    int include_retrograde,
    double* retrograde_records,
    int* retrograde_noreg_flags,
    int* status
) noexcept {
    const bool use_retrograde = include_retrograde != 0;
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        psi_obs_degrees == nullptr || black_hole_mass < 0.0 ||
        black_hole_softening_arcsec < 0.0 || dark_halo_parameter_count < 0 ||
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        energy_count <= 0 || i2_count <= 0 || i3_count <= 0 ||
        inner_boundaries == nullptr || middle_boundaries == nullptr ||
        outer_boundaries == nullptr || irregular == nullptr || noreg_grid == nullptr ||
        theta_values == nullptr || energies == nullptr || circular_periods == nullptr ||
        circular_radii == nullptr || circular_velocities == nullptr || records == nullptr ||
        noreg_flags == nullptr ||
        (use_retrograde && (retrograde_records == nullptr || retrograde_noreg_flags == nullptr))) {
        set_status(status, kStatusInvalidArgument);
        return;
    }
    ...
    try {
        ...
        if (!dynamite::orblib_cpp::build_tube_start_records(
                mge,
                halo,
                black_hole_mass,
                black_hole_softening_arcsec * mge.conversion_factor,
...
extern "C" void orblib_cpp_api_orbitstart_tube_orbit_width(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        n_radius < 2 || n_theta < 2 || n_phi < 2 || rlogmax <= rlogmin ||
        radius <= 0.0 || circular_period <= 0.0 || plane < 1 || plane > 3 ||
        integrator_accuracy <= 0.0 || crossing_capacity <= 0 ||
        crossing_positions == nullptr || width == nullptr || crossing_count == nullptr ||
        solver_status == nullptr || function_evaluations == nullptr ||
        !std::isfinite(radius) || !std::isfinite(start_theta) ||
    ...
    try {
        ...
        int result_crossing_count = 0;
        int result_solver_status = 0;
        int result_function_evaluations = 0;
        if (!dynamite::orblib_cpp::measure_tube_orbit_width(
                interpolated,
                radius,
                start_theta,
                energy,
                ...
                crossing_positions,
                result_width,
                result_crossing_count,
                result_solver_status,
                result_function_evaluations
            )) {
            set_status(status, kStatusInvalidArgument);
            return;
        }
        *width = result_width;
        *crossing_count = result_crossing_count;
        *solver_status = result_solver_status;
        *function_evaluations = result_function_evaluations;
        set_status(status, kStatusOk);
    } catch (...) {
        set_status(status, kStatusException);
    }
...
extern "C" void orblib_cpp_api_orbitstart_find_tube_radius(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        n_radius < 2 || n_theta < 2 || n_phi < 2 || rlogmax <= rlogmin ||
        inner_radius < 0.0 || middle_radius <= inner_radius ||
        middle_radius >= outer_radius || outer_radius <= 0.0 || circular_period <= 0.0 ||
        plane < 1 || plane > 3 || integrator_accuracy <= 0.0 || crossing_capacity <= 0 ||
        radius == nullptr || width == nullptr || width_evaluations == nullptr ||
        crossing_count == nullptr || solver_status == nullptr ||
        function_evaluations == nullptr || !std::isfinite(inner_radius) ||
        !std::isfinite(middle_radius) || !std::isfinite(outer_radius) ||
        !std::isfinite(start_theta) || !std::isfinite(energy) ||
        !std::isfinite(circular_period)) {
        set_status(status, kStatusInvalidArgument);
        return;
    }
    ...
    try {
        ...
        double result_radius = 0.0;
        double result_width = 0.0;
        int result_width_evaluations = 0;
        int result_solver_status = 0;
        int result_crossing_count = 0;
        int result_function_evaluations = 0;
        if (!dynamite::orblib_cpp::find_tube_radius(
                interpolated,
                inner_radius,
                middle_radius,
                outer_radius,
        ...
        *radius = result_radius;
        *width = result_width;
        *width_evaluations = result_width_evaluations;
        *crossing_count = result_crossing_count;
        *solver_status = result_solver_status;
        *function_evaluations = result_function_evaluations;
        set_status(status, kStatusOk);
...
extern "C" void orblib_cpp_api_orbitstart_find_type(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        n_radius < 2 || n_theta < 2 || n_phi < 2 || rlogmax <= rlogmin ||
        radius <= 0.0 || circular_period <= 0.0 || integrator_accuracy <= 0.0 ||
        sample_count <= 0 || orbit_type == nullptr || samples_collected == nullptr ||
        solver_status == nullptr || function_evaluations == nullptr ||
        !std::isfinite(radius) || !std::isfinite(start_theta) ||
        !std::isfinite(energy) || !std::isfinite(circular_period)) {
        set_status(status, kStatusInvalidArgument);
    ...
    try {
        ...
        int result_orbit_type = 5;
        int result_samples_collected = 0;
        int result_solver_status = 0;
        int result_function_evaluations = 0;
        if (!dynamite::orblib_cpp::find_orbit_type(
                interpolated,
                radius,
                start_theta,
                energy,
        ...
        *orbit_type = result_orbit_type;
        *samples_collected = result_samples_collected;
        *solver_status = result_solver_status;
        *function_evaluations = result_function_evaluations;
        set_status(status, kStatusOk);
    } catch (...) {
...
extern "C" void orblib_cpp_api_orbitstart_prepare_grid(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        n_radius < 2 || n_theta < 2 || n_phi < 2 || rlogmax <= rlogmin ||
        energy_count <= 1 || i2_count <= 3 || integrator_accuracy <= 0.0 ||
        crossing_capacity <= 0 || type_sample_count <= 0 || circular_radii == nullptr ||
        circular_velocities == nullptr || circular_periods == nullptr || energies == nullptr ||
        theta_values == nullptr || outer_boundaries == nullptr || inner_boundaries == nullptr ||
        irregular == nullptr || inner_orbit_types == nullptr ||
        equivalent_radius_iterations == nullptr || inner_width_evaluations == nullptr ||
        inner_type_function_evaluations == nullptr) {
        set_status(status, kStatusInvalidArgument);
        return;
    }
    ...
    try {
        ...
        dynamite::orblib_cpp::OrbitStartGridDiagnostics diagnostics;
        if (!dynamite::orblib_cpp::prepare_orbit_start_grid(
                mge,
                halo,
                black_hole_mass,
                black_hole_softening_km,
        ...
        *equivalent_radius_iterations = diagnostics.equivalent_radius_iterations;
        *inner_width_evaluations = diagnostics.inner_width_evaluations;
        *inner_type_function_evaluations = diagnostics.inner_type_function_evaluations;
        set_status(status, kStatusOk);
    } catch (...) {
        set_status(status, kStatusException);
    }
...
extern "C" void orblib_cpp_api_orbitstart_inner_boundaries(
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        n_radius < 2 || n_theta < 2 || n_phi < 2 || rlogmax <= rlogmin ||
        energy_count <= 0 || i2_count <= 3 || outer_boundaries == nullptr ||
        energies == nullptr || circular_periods == nullptr || theta_values == nullptr ||
        integrator_accuracy <= 0.0 || crossing_capacity <= 0 || type_sample_count <= 0 ||
        inner_boundaries == nullptr || irregular == nullptr || orbit_types == nullptr ||
        width_evaluations == nullptr || type_function_evaluations == nullptr) {
        set_status(status, kStatusInvalidArgument);
        return;
    }
    ...
    try {
        ...
        int result_width_evaluations = 0;
        int result_type_function_evaluations = 0;
        if (!dynamite::orblib_cpp::find_inner_boundaries(
                interpolated,
                energy_count,
                i2_count,
                outer_boundaries,
                ...
                orbit_types,
                result_width_evaluations,
                result_type_function_evaluations
            )) {
            set_status(status, kStatusInvalidArgument);
            return;
        }
        *width_evaluations = result_width_evaluations;
        *type_function_evaluations = result_type_function_evaluations;
        set_status(status, kStatusOk);
    } catch (...) {
        set_status(status, kStatusException);
    }
...
extern "C" void orblib_cpp_api_orbitstart_outer_boundaries(
    int ngauss,
    const double* surf_pc,
    const double* sigobs_arcsec,
    const double* qobs,
    ...
    if (ngauss <= 0 || surf_pc == nullptr || sigobs_arcsec == nullptr || qobs == nullptr ||
        ...
        (dark_halo_parameter_count > 0 && dark_halo_parameters == nullptr) ||
        n_radius < 2 || n_theta < 2 || n_phi < 2 || rlogmax <= rlogmin ||
        energy_count <= 0 || i2_count <= 3 || i3_count <= 0 ||
        ...
        irregular == nullptr || orbit_types == nullptr || width_evaluations == nullptr ||
        type_function_evaluations == nullptr) {
        set_status(status, kStatusInvalidArgument);
        return;
    }
    ...
    try {
        ...
        int result_width_evaluations = 0;
        int result_type_function_evaluations = 0;
        if (!dynamite::orblib_cpp::find_outer_boundaries(
                interpolated,
                energy_count,
                i2_count,
                i3_count,
