...
module triaxpotent
    ...
    public:: tp_setup
    ! Inactive bar setup is kept below as comments.
    ! The active direct shared-library path is non-bar triaxial.
...
contains
    ...
    subroutine tp_setup()
        ...
        sigintr_km(:) = sigobs_km(:)*sqrt(qobs(:)/sqrt((pintr(:)*cos(theta_view))**2 &
                                                       + (qintr(:)*sin(theta_view))**2 &
                                                       *((pintr(:)*cos(phi_view))**2 + sin(phi_view)**2)))
    ...
    !+++++++++++++++++++++++++++++++++++
    !+++++++++++++++++++++++++++++++++++++
    ! INACTIVE LEGACY ROUTINE: Inactive legacy bar potential setup: active direct shared-library path is non-bar triaxial.
...
!     end subroutine tp_setup_bar
    ...
    subroutine accin(n, x, y, z, vx, vy, vz)
        ...
        vx = -V0(n)/sqrt(1.0 - qintr(n)**2)*x/sigintr_km(n)**2* &
             (A1(n) - 1.0_dp/(2.0_dp*sigintr_km(n)**2)*(A11*x2 + A12*y2 + A31*z2))

        vy = -V0(n)/sqrt(1.0 - qintr(n)**2)*y/sigintr_km(n)**2* &
             (A2(n) - 1.0_dp/(2.0_dp*sigintr_km(n)**2)*(A12*x2 + A22*y2 + A23*z2))

        vz = -V0(n)/sqrt(1.0 - qintr(n)**2)*z/sigintr_km(n)**2* &
             (A3(n) - 1.0_dp/(2.0_dp*sigintr_km(n)**2)*(A31*x2 + A23*y2 + A33*z2))
...
contains
    ...
    !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! INACTIVE LEGACY ROUTINE: Inactive legacy file-reading helper: active Python path passes arrays directly.
    ! Retained as comments for reference; do not call from active direct ABI path.
