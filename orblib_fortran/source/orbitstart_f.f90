!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! Note about rotating bars:
! The modified or added lines are commented by (BT)
! In this file, the sampling of initial conditions in case of rotation is modified.
! If  (Omega = 0) it will be the same as before, two-orbit libraries including tube and box orbits.
! If  (Omega != 0), we will integrate orbits in a rotating frame (prograde and retrograde orbits are not identical).
! So we still have two-orbit libraries; the tube orbit library is the same as before, but instead of 
! box orbits initial conditions,  the same initial conditions as tube orbits 
! will generate but with -Vy to produce retrograde orbits.
! Still, the name of this library is "beginbox.dat" orbits to consist of other parts of the code. 
!
! Note: the sampling of initial conditions is still in a stationary frame.
!
...
contains
    ...
    subroutine runorbitstart()
        ...
        rcirc(:) = 10.0_dp**rcirc(:)
        print *, 'Radii of the energies in arcsec:'
        ...
        if (Omega == 0.0_dp ) then     ! (BT)  make box orbits same as before if only omega=0
...
! the box orbits
    ...
    subroutine make_startpoints(boundin, boundmid, boundout, irregular &
        ...
        real(kind=dp) :: t1
        real(kind=dp), dimension(6):: startpos, startpos2
        integer(kind=i4b) :: i, j, k
           ...
           do i=1,NEner
              ...
              do j=1,nI2
                 do k=1,nI3
                    ...
                    startpos2=startpos                 ! make negative Vy for retrograte orbits (! BT)
                    startpos2(5)=-1*startpos2(5)
            ...
            if (notubes == 0 .and. i > 1) then
                ...
                if (i > 2) then
                    ...
                    do k = i - 1, 1, -1
                        ...
                        if (rbi .ge. rbu) then
                            print *, 'inner boundary ', rbi, ' bigger than outer boundary ', rbu
                            stop 'inner boundary bigger than outer boundary'
                        end if
                        ...
                        print "(3I5 , 4F7.3)", j, k, o1, boundin(j, k)/boundout(j, Ni2), &
                            rel_rbi*boundout(j, k)/boundout(j, Ni2), &
