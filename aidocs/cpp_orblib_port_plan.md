...
- The `findtube()` golden-section tube-radius minimizer is ported as
  `find_tube_radius()`. It preserves the Fortran `0.61803399` ratio, initial
  bracket placement around `Rmid`, `T2 < T1` branch rule, and
  `abs(R3-R0) < 1e-4*(abs(R1)+abs(R2))` stopping condition. The minimizer
  calls `measure_tube_orbit_width()` with no crossing-position buffer, so trial
  radii do not allocate or copy crossing arrays. ABI tests mirror the Fortran
  golden-section loop and use the already SciPy-tested tube-width ABI as the
  trial-width oracle.
- The `find_type()` orbit-start probe is ported as `find_orbit_type()`. It
  preserves the non-rotating interpolated-acceleration RHS, `100*tcirc`
  integration length, `XEND/(intsteps+4)` dense-output sample spacing, DOP853
  tolerance setup, and angular-momentum sign-product classification rules. The
  C++ implementation streams angular-momentum min/max values during dense
  sampling instead of allocating full `pos_t`/`vel_t` arrays for each probe.
  ABI tests compare the sampled classifier against an independent SciPy DOP853
  direct black-hole orbit where the C++ interpolated-potential path falls back
  to direct acceleration.
- The `find_innerboundary()` short-axis-tube inner-boundary loop is ported as
  `find_inner_boundaries()`. It preserves the reverse `nI2` loop order, first
  and second guess formulas, previous-energy scaling for the last angular
  cell, edge-near irregular flagging, and Fortran's actual `otype == 1`
...
- The full memory-side `runorbitstart()` start-array orchestration is ported
  as `build_orbit_start_arrays()`. It composes the prepared grid,
  triaxiality branch decision, `find_outerboundary()` middle-boundary search,
  irregular-energy dithering expansion, `find_unregorbits()`, tube
  begin-record generation, `Omega != 0` retrograde beginbox generation, and
  `Omega == 0` box beginbox generation. The C ABI helper
